...
@section('content')
    ...
    <div class="section-full content-inner">
        <!-- Product -->
        <div class="container">
            <form class="shop-form" method="post" enctype="multipart/form-data" action="{{ route('updateCar') }}">
                @csrf
                <div class="row">
                        ...
                        <div class="row">
                            <div class="form-group col-lg-6 col-md-6 col-sm-6">
                                <input type="hidden" name="id"
                                       value="{{ \Illuminate\Support\Facades\Crypt::encryptString($car['id']) }}">
                                <input type="hidden" name="id_cliente"
                                       value="{{ \Illuminate\Support\Facades\Crypt::encryptString($car['id_cliente']) }}">
                                <span class="font-weight-bold text-danger">{{ trans('messages.brand') }}</span>
                                <select
                                    {{ session('userLogged')['id_tipo_usuario']==3 || session('userLogged')['id_tipo_usuario']==4 ? 'disabled' : '' }} class="form-control"
                                    id="marca" name="marca">
                                    <option value="selectBrand">{{ trans('messages.selectBrand') }}</option>
                                    @foreach($brands as $b)
                                        <option
                                            value="{{ $b['id'] }}" {{ old('marca') == $b['id'] || $car['marca'] == $b['nombre'] ? 'selected' : '' }}>{{ $b['nombre'] }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('marca'))
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-lg-6 col-md-6 col-sm-6">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('anio') ? old('anio') : ucfirst($car['anio']) }}" name="anio">
                                @if($errors->has('anio'))
                        ...
                        <div class="row">
                            <div class="form-group col-lg-6 col-md-6 col-sm-6">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('matricula') ? old('matricula') : strtoupper($car['matricula']) }}"
                                       name="matricula">
                            ...
                            <div class="form-group col-lg-6 col-md-6 col-sm-6">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('bastidor') ? old('bastidor') : strtoupper($car['bastidor']) }}"
                                       name="bastidor">
                            ...
                            <div class="row">
                                <div class="form-group">
                                    <button class="site-button button-lg btn-block ml-4"
                                            type="submit">{{ trans('messages.updateCar') }}</button>
                                </div>
                            </div>
                        @endif
                    </div>
                    <div class="col-md-6 col-lg-6 m-b30"
                         style="background: center center no-repeat url('{{ assetFtp('images/viewCar.jpg') }}'); background-size:
                             contain">
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            if (selectedBrand != "selectBrand") {
                ...
                if (oldModel == "") {
                    oldModel = "{{ $car['id_modelo'] }}";
                }

                if (oldEngine == "") {
                    oldEngine = "{{ $car['id_motor'] }}";
                }
